...
$results  = "slx_"."$ARGV[0]"."_correct_reads.txt";
open(OUTPUT2, ">>$results");

$results  = "slx_"."$ARGV[0]"."_snp_read_counts.txt";
open(OUTPUT, ">>$results");
...
open (INPUT2, "slx"."_$ARGV[0]"."_filenames.txt") or die "\nFilenames file could not be found\n\n";
while (<INPUT2>){
	chomp ($_);
	$sample = $_;
	push(@SAMPLE,$sample);
	...
	while ($index < scalar@RS_ID){
		...
		while (<INPUT>){
			chomp ($_);
			if (($_ =~ /^$FOR_PRIMER[$index]/) && ($_ =~ /$REV_PRIMER[$index]$/)){
